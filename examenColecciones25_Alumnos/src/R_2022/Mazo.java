...
public class Mazo {
	 ...
	 * valor el número de unidades del mismo que tenemos en el mazo.
	 * Cuidado porque al hacer Cromo clave, tenemos que codificar 
	 * el método equals en la clase Cromo.
	 */
	 ...
	 * @param numeroCromos el número de unidades del cromo c que voy a añadir al mazo
	 * @return el número actual de unidades del cromo c resultante
	 */
	...
	/**
	 * Coge un cromo (de otro mazo, pero no se trata eso aquí) y lo añade a mi mazo.
	 * Al mismo tiempo, elimina una unidad de uno de mis cromos. Si fuera la última
	 * unidad, se ha de eliminar el cromo también.
	...
	public void intercambiar(Cromo mio, Cromo nuevo) throws MazoException {
		...
		if (this.mazo.get(mio).intValue() > 1) {
			// Tengo más de uno, por tanto el cromo se quedará en el mazo
			this.mazo.put(mio, Integer.valueOf(this.mazo.get(mio).intValue() - 1));
		}
		else {
			// Tan solo tengo 1, por lo que habrá que eliminarse del mazo
			this.mazo.remove(mio);
		}
		
		// Por último añadimos el nuevo cromo al mazo
		this.addCromo(nuevo);
	}
	
	/**
	 * Mezcla mi mazo con otro que recibo como parámetro
	 * @param otro el mazo con el que voy a mezclar el mío
	 */
	 ...
	 * el número de cromos distintos que tengo
	 * @return
	 */
	...
	public List<Cromo> cromosDeUnEquipo(String equipo) {
		/*
		 * Filtramos los cromos del mazo. Solo pasarán el filtro aquellos escudos
	...
	public List<String> equipoCompleto(){
		...
		// Primero obtengo los escudos, para después ver si tengo todos sus jugadores.
