...
public class SimuladorEstacionAlumnos {
    ...
    public static void main(String[] args) throws SimuladorException {
        ...
        for (ModuloEstacion m : modulos) {
            ...
            // No funciona bien por lo que esta desactivada
            /*
            * if (m instanceof BateriaRespaldo) {

                if (m.getEnergiaDisponible() <= 50) {
                    m.setEnergiaDisponible(m.getEnergiaDisponible() + 30);

                    System.out.printf("[SISTEMA] Activando baterías auxiliares en" + m.getNombre() );
                }
            }
